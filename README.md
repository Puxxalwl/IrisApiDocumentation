...
**Iris API использует HTTP запросы в формате https://iris-tg.ru/api/{bot_id}_{Iris-Token}/{method}?{params}** возвращающий JSON. Далее нам предстоит работать с этим

---


## Проверка
Изначально проверьте работает ли у вас Api простым запросом (можно даже в браузере) `https://iris-tg.ru/api/{bot_id}_{Iris-Token}/pocket/balance` если ответ будет наподобии:
```JSON
